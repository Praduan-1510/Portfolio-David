...
prototype:
  ...
  aspect: 1.6
  launchNote: >-
    It starts where a real user would: a splash, three intro cards, then a
    passwordless sign in. There is no password to type and the six-digit code
    is already filled, so you can walk the whole way in or skip ahead.
  surfaces:
    # Entry point is the first-run flow, not the signed-in dashboard: onboarding
    # is where this product makes its argument about households and consent, and
    # auth.html was otherwise a surface a visitor would never see. It links on to
    # app.html at the end, so the flow completes into the product.
    - id: patient
      label: Patient
      shortLabel: Patient
      src: /prototype/carebridge/auth.html
...
outcome: "132 clickable screens across patient, clinician and operations, on one token system."
...
<Margin label="Try this">
  Launch the demo and, on a wider screen, use the device buttons in its top bar. They resize the prototype's real layout viewport rather than zooming a picture, so you are watching its own breakpoints fire, not a simulation of them.
</Margin>
